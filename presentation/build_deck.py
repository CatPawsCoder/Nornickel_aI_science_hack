...
txt(s, 0.9, 4.35, 10.5, 0.6,
    "2 090 документов корпуса кейса · граф знаний, где числа не может исказить ни одна "
    "модель — они проверяются дословным совпадением с источником.", size=15, italic=True,
    color=STEEL, font=FONT_BODY)
# нижняя строка с фактами
facts = ["2 090 публикаций", "292 155 условий", "665 утверждений", "100% верификация"]
fx = 0.9
for f in facts:
    w = 0.16 * len(f) + 0.5
...
txt(s, 0.7, 5.0, 11.5, 1.5,
    "665 из 665 извлечённых LLM утверждений прошли string-match верификацию цитаты к исходному тексту документа.\n"
...
stack = [
    ("Ингест", "PyMuPDF, docx, xls, pptx"),
    ("OCR сканов", "easyocr на GPU (RTX 5090)"),
    ...
    ("LLM-инференс", "Yandex → OpenRouter → GigaChat"),
...
status = [
    ("2 090", "документов обработано"),
    ("292 155", "числовых условий"),
    ("665", "утверждений (100% verified)"),
    ("3", "LLM-провайдера"),
]
sx2 = 0.9
