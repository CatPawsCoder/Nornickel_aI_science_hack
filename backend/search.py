...
def search(query: str, idx: dict, conn=None, top_chunks: int = 12) -> dict:
    ...
    # генерик-слова обзорных формулировок намерением НЕ считаются — иначе
    # «обзор способов удаления SO2 на предприятиях мира» ложно срабатывал
    # на «предприят»/«металлургическ»/«удален» (стем-разрыв «удаления»/«удаляют»)
    GENERIC_INTENT = {
        "обзор", "литературн", "способ", "метод", "технолог", "техническ",
        "решени", "практик", "предприят", "металлургическ", "промышленн",
        "производств", "организаци", "информаци", "анализ", "пример",
        "мирово", "отечествен", "зарубежн", "российск", "удален", "удалени",
        "провест", "выполн", "существующ", "область", "получен", "использован",
        # аспект-слова инженерных вопросов (грани темы, а не сама тема)
        "подач", "вывод", "движени", "движен", "конструкци", "поток",
        "раствор", "пространств", "раздел", "оптимальн", "скорост", "сравнен"}
    missing_intent = []
    if conn is not None and qtok_all:
        pool_text = " ".join(c["text"].lower() for c in (claims + claims_out))
        pool_tok = set(tokenize(pool_text))
        for t in qtok_all:
            if len(t) >= 5 and t not in pool_tok and t not in GENERIC_INTENT:
