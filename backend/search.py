...
def search(query: str, idx: dict, conn=None, top_chunks: int = 12) -> dict:
    ...
    # WHITELIST intent-якорей: «данных нет» может вызвать только заранее
    # известная доменная операция, а не произвольное слово запроса.
    # (Blacklist генериков был хрупким: «металлургический», «литературный»,
    # «современный» ложно объявлялись отсутствующими темами.)
    INTENT_ANCHORS = {
        "закачк": "закачка в глубокие горизонты",
        "нагнетан": "нагнетание в пласт",
        "захоронен": "подземное захоронение",
        "закладк": "закладка выработанного пространства",
        "рекультивац": "рекультивация",
    }
    missing_intent = []
    if conn is not None and qtok_all:
        pool_text = " ".join(c["text"].lower() for c in (claims + claims_out))
        pool_tok = set(tokenize(pool_text))
        for stem_a, label in INTENT_ANCHORS.items():
            if any(t.startswith(stem_a) for t in qtok_all) and \
                    not any(t.startswith(stem_a) for t in pool_tok):
                missing_intent.append(label)
