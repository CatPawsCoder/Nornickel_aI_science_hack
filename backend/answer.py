...
def _fmt_constraint(f: dict) -> str:
    v2 = f.get("value2")
    if v2 not in (None, -1.0):
        val = f"{f['value']:g}–{v2:g}"
    else:
        op = _OP_PRETTY.get(f["op"], f["op"])
        val = f"{op}{f['value']:g}" if op else f"{f['value']:g}"
...
_NEG_RE = _re.compile(r"не\s+(?:подход|удаля|примен|эффект|способн)|неэффективн|нельзя", _re.I)
_NODE_REF_RE = _re.compile(r"\[узел\s+([\w-]+)\]")


def _negation_substances_ok(answer: str, facts_by_id: dict) -> bool:
    """Пред-предложенная проверка отрицаний: «X не подходит для удаления Y» —
    вещества Y обязаны фигурировать в самом факте, на который ссылается
    [узел ...] предложения. Ловит подмену «натрий и хлор» -> «хлориды и сульфаты»."""
    for sent in _re.split(r"(?<=[.!?])\s+|\n", answer):
        if not _NEG_RE.search(sent):
            continue
        refs = _NODE_REF_RE.findall(sent)
        if not refs:
            continue
        fact_text = " ".join(
            (facts_by_id.get(r, {}).get("text", "") + " " +
             (facts_by_id.get(r, {}).get("quote") or "")) for r in refs)
        if not fact_text.strip():
            continue
        if not _materials_of(sent) <= _materials_of(fact_text):
            return False
    return True


def _materials_of(text: str) -> set:
    """Канонические материалы/вещества тезауруса, упомянутые в тексте.
    Используется для валидации: LLM не имеет права вводить в ответ вещество,
    которого нет ни в фактах, ни в вопросе (ловит подмену «натрий и хлор» ->
    «хлориды и сульфаты» там, где вещество не фигурировало вовсе)."""
    global _matchers_cache
    from thesaurus import build_matcher, spot
    if _matchers_cache is None:
        _matchers_cache = build_matcher()
    return {cid for (etype, cid) in spot(text, _matchers_cache) if etype == "Material"}
...
def _direct_answer(question: str, claims: list, conds: list,
    ...
    intent_note = ""
    if missing_intent:
        terms = ", ".join(f"«{t}»" for t in missing_intent[:3])
    ...
    llm_ans = llm.complete(
        ...
        "(7) ЗАПРЕЩЕНО заменять или расширять химические вещества факта: если в факте сказано "
        "«натрий и хлор», нельзя писать «хлориды и сульфаты» — перечисляй ровно те вещества, "
        "что названы в самом факте, особенно в отрицаниях («не удаляет», «не подходит»); "
        "(8) не делай выводов о целесообразности/применимости, которых нет в фактах дословно."
        + intent_note + compare_note +
        " Не добавляй ничего от себя.",
        f"Вопрос: {question}\n\nФакты из графа знаний:\n{facts_blob}\n{cond_blob}\n{chunk_blob}",
        temperature=0.0, max_tokens=650)  # лимит генерации: держим латентность ~3-5с
    allowed_materials = _materials_of(facts_blob + " " + cond_blob + " " +
                                      chunk_blob + " " + question)
    facts_by_id = {c["id"]: c for c in top}

    def _valid(ans: str) -> bool:
        # 1: числа ответа обязаны существовать в фактах
        # 2: вещества ответа — в фактах или вопросе
        # 3: отрицания не приписывают факту чужие вещества (по [узел ...])
        return (_numbers_of(ans) <= allowed_numbers and
                _materials_of(ans) <= allowed_materials and
                _negation_substances_ok(ans, facts_by_id))
...
def synthesize(result: dict, docs_meta: dict) -> dict:
    ...
    mi = result.get("missing_intent") or []
    if mi:
        md.append(f"- ⚠️ Пробел корпуса: тема «{mi[0]}» в базе утверждений не представлена — "
                  f"показаны смежные материалы")
